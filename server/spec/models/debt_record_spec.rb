  ...
  describe '#check_payment_is_completed_when_destroying' do
    ...
    context 'when the payment is completed' do
      ...
      it 'includes an error' do
        debt_record = Expense.first.debt_records.first
        debt_record.destroy
        expect(debt_record.errors[:base]).to include '返済が完了しているため削除できません'
      end

      it 'does not destroy the debt record' do
        expect { Expense.first.debt_records.first.destroy }.not_to change(described_class, :count)
      end
    end
    ...
    context 'when the payment is not completed' do
      ...
      it 'destroys the debt record' do
        expect { Expense.first.debt_records.first.destroy }.to change(described_class, :count).by(-1)
      end
    end
  end
end
